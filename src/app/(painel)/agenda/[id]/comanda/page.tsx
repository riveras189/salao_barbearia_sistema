...
export default async function AbrirComandaPorAgendaPage({
  params,
}: {
  ...
  const agenda = await prisma.agendamento.findFirst({
    where: {
      id,
  ...
  if (agenda.comandaId) {
    redirect(`/comandas/${agenda.comandaId}`);
  }
  ...
  const criada = await prisma.$transaction(async (tx) => {
    ...
    const comanda = await tx.comanda.create({
      data: {
        ...
        profissionalPrincipalId: agenda.profissionalId,
    ...
    for (const servico of agenda.servicos) {
      ...
      const valorComissao = money2((valorTotal * comissaoPercentual) / 100);
